...
BuildLinearData <-
  function (data ,
    ...
    PM.end <- 54780 # 15:13
    start.time <-
      ifelse (morning , AM.start , PM.start) # - data start
    
    open.time <-
      ifelse (morning , AM.open , PM.open) # - trade open
    close.time <-
      ifelse (morning , AM.close , PM.close) # - trade close
    ...
    time.secs <-
      main.data$SecondOfDay + main.data$UpdateMillisec / 1000
    ind.open <- head (which (time.secs >= open.time) , 1)
    ind.close <- head (which (time.secs >= close.time) , 1)
    
    # calculate variables
    mid.price <-
      (main.data$BidPrice1 + main.data$AskPrice1) / 2
    ...
    AvgTrade.price <-
      na.locf (na.locf (AvgTrade.price , na.rm = F), fromLast = T)
    MPB.array <-
      (AvgTrade.price - c(mid.price [1] , rollmean (mid.price , k = 2)))
    ...
    if (k > 0) {
      library (zoo)
      fpc <-
        rollmean (mid.price , k = k)[-1] - mid.price [1:(n - k)]
